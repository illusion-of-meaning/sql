...
SELECT first_name AS "Имя", SUM(age) AS "Суммарный возраст" FROM employee e GROUP BY first_name;

-- Get min age for each name
SELECT first_name AS "Имя", MIN(age) AS "Минимальный возраст" FROM employee e GROUP BY first_name; 

-- Output the name and maximum age only for non-unique names. Sort the result by age in ascending order.
SELECT first_name AS "Имя", MAX(age) AS "Максимальный возраст" FROM employee 
WHERE first_name IN (
